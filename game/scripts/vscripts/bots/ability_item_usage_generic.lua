...
--  print("--> ",npcBot:GetUnitName()," run once")
  ...
  elseif npcBot:GetUnitName() == "npc_dota_hero_kunkka" then
    FirstAbility = 0
    SecondAbility = 1
  elseif npcBot:GetUnitName() == "npc_dota_hero_legion_commander" then
    FirstAbility = 0
    SecondAbility = 1
...
local function LevelUp()
  ...
  for abilityIndex = 3,9,1 do
    ...
    if hAbility ~= nil then
      if hAbility:IsUltimate() and hAbility:CanAbilityBeUpgraded() and npcBot:GetAbilityPoints() > 0 then
...
--        print("--> ",npcBot:GetUnitName()," levelled up ",hAbility:GetName()," at level ",npcBot:GetLevel())
  ...
  for abilityIndex = 9,23,1 do
    hAbility = npcBot:GetAbilityInSlot( abilityIndex )
    if hAbility ~= nil then
      if hAbility:CanAbilityBeUpgraded() and npcBot:GetAbilityPoints() > 0 then
        npcBot:ActionImmediate_LevelAbility (hAbility:GetName())
...
--  if SecondAbility ~= -1 then
  ...
  if hAbility:CanAbilityBeUpgraded() and npcBot:GetAbilityPoints() > 0 and hAbility:GetLevel() <= 0 then
    npcBot:ActionImmediate_LevelAbility (hAbility:GetName())
