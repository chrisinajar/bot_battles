...
function AbilityUsageThink()
  ...
  if abilityCH == nil then abilityCH = npcBot:GetAbilityByName( "night_stalker_void" ) end
  if abilityCF == nil then abilityCF = npcBot:GetAbilityByName( "night_stalker_crippling_fear" ) end
  ...
  castCHDesire, castCHTarget = ability_item_usage_generic.ConsiderAOERadiusAbility(npcBot, abilityCH);
  castCFDesire = ability_item_usage_generic.ConsiderAOERadiusAbility(npcBot, abilityCF);
  castEDesire = ConsiderE();
  castWBDesire = ConsiderWildBoar();

  if ( castCH1Desire > 0 and not npcBot:HasScepter() )
  then
    npcBot:Action_UseAbilityOnEntity( abilityCH1, castCH1Target );
    return;
  end

  if ( castCHDesire > 0 and npcBot:HasScepter() )
  then
  ...
  if ( castCFDesire > 0 )
  then
    npcBot:Action_UseAbility( abilityCF );
    return;
  end
...
function IsNightTime()
  return GetTimeOfDay() == 0.0
    or npcBot:HasModifier("modifier_night_stalker_hunter_in_the_night")
...
function ConsiderWildBoar()
  ...
  if #tableNearbyEnemyHeroes > 0 and not IsNightTime() then
