...
function ConsiderOvergrowth()
  ...
  then
    ...
    if mutil.IsValidTarget(npcTarget)
      ...
      and mutil.IsInRange(npcTarget, npcBot, 1200)
      and not npcTarget:HasModifier("modifier_winter_wyvern_winters_curse")
      and not npcTarget:HasModifier("modifier_winter_wyvern_winters_curse_aura")
    then
      return BOT_ACTION_DESIRE_HIGH;
    end
  end
...
function ConsiderIgnite()
  ...
  then
    ...
    then
      ...
      do
        if h ~= npcTarget
          and mutil.CanCastOnNonMagicImmune(h)
          and not h:HasModifier("modifier_winter_wyvern_winters_curse")
          and not h:HasModifier("modifier_winter_wyvern_winters_curse_aura")
        then
