...
function U.GetUnitRange(hero)
  local range = hero:GetAttackRange()

  for abilityIndex = 0, hero:GetAbilityCount() - 1 do
    local ability = hero:GetAbilityByIndex(abilityIndex)
    if ability and ability:GetCastRange() > range then
  ...
  for itemIndex = DOTA_ITEM_SLOT_1, DOTA_ITEM_SLOT_6 do
    ...
    if item and item:GetCastRange() > range then
      range = item:GetCastRange()
    end
  end
