...
function AbilityUsageThink()
	...
	if abilityDF == nil then abilityDF = npcBot:GetAbilityByName( "chen_divine_favor" ) end
	...
	castDFDesire, castDFTarget = ConsiderDivineFavor();
  ...
  if ability_item_usage_generic.ThinkAboutMindControl(abilityHP, "modifier_chen_holy_persuasion") then
    return
  end
  ...
  if ( castDFDesire > 0 )
  then
    npcBot:Action_UseAbilityOnEntity( abilityDF, castDFTarget );
    return;
  end
...
function ConsiderHolyPersuasion()
  local maxHP = 0;
  local NCreep = nil;
  local tableNearbyNeutrals = npcBot:GetNearbyNeutralCreeps( 1600 );
...
function ConsiderDivineFavor()
	...
	for _,npcAlly in pairs( tableNearbyAllies )
	do
		if ( mutil.CanCastOnNonMagicImmune(npcAlly) and npcAlly ~= npcBot )
		then
			local nAllyHP = npcAlly:GetHealth();
			if  nAllyHP < nLowestHealth and npcAlly:GetHealth() / npcAlly:GetMaxHealth() < 0.25 and npcAlly:WasRecentlyDamagedByAnyHero(3.0)
			then
