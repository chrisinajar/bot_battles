...
local function ConsiderQ()
  ...
  local enemies = bot:GetNearbyHeroes(nRadius + 100, true, BOT_MODE_NONE);

  if enemies ~= nil and #enemies > 0 then
    for i=1,#enemies do
      if IsValidObject(enemies[i]) and GetUnitToUnitDistance(bot, enemies[i]) < nCastRange and enemies[i]:IsChanneling()
      then
...
local function ConsiderW()
  ...
  local enemies = bot:GetNearbyHeroes(math.max(nRadius, nCastRange) + 100, true, BOT_MODE_NONE);
  ...
  if GetUnitCountWithinRadius(enemies, nRadius) >= 2
  then
