...
function AbilityUsageThink()
  ...
  if abilitySA:IsTrained() and not abilitySA:GetAutoCastState( )  then
  ...
  if ( castEADesire > 0 )
  then
    npcBot:Action_UseAbility( castEADesire );
    return;
  end
  if ( castDRDesire > 0 )
  then
...
function ConsiderStaticStorm()
  ...
  then
    ...
    if npcBot:GetHealth() / npcBot:GetMaxHealth() < 0.2 then
      return BOT_ACTION_DESIRE_MODERATE, npcBot:GetTarget():GetLocation();
    end
  end
